        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '预约编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.changdiYuyueUuidNumber" placeholder="预约编号" clearable></el-input>
                     </el-form-item>
                                                 
                     <el-form-item :label="contents.inputTitle == 1 ? '预约状态' : ''">
                        <el-select v-model="searchForm.changdiYuyueYesnoTypes" placeholder="请选择预约状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in changdiYuyueYesnoTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('changdiYuyue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jianshenfangguanli/upload/changdiYuyueMuBan.xls"
                        >批量导入场地预约申请数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('changdiYuyue','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="changdiYuyueUploadSuccess"
                                :on-error="changdiYuyueUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('changdiYuyue','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入场地预约申请数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('changdiYuyue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "changdiYuyue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="changdiPhoto"
                        ...
                        <template slot-scope="scope">
                            <div v-if="scope.row.changdiPhoto">
                                <img :src="$base.url+scope.row.changdiPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="changdiYuyueUuidNumber"
                                   header-align="center"
                                   label="预约编号">
                        <template slot-scope="scope">
                            {{scope.row.changdiYuyueUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="sheshiyuyueText"
                                      header-align="center"
                                      label="备注">
                        <template slot-scope="scope">
                            <span v-if="scope.row.sheshiyuyueText != null &&scope.row.sheshiyuyueText.length>10">
                                {{scope.row.sheshiyuyueText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.sheshiyuyueText}}
                            </span>
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="changdiYuyueTime"
                                   header-align="center"
                                   label="开始时间">
                        <template slot-scope="scope">
                            {{scope.row.changdiYuyueTime}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="changdiJieshuTime"
                                   header-align="center"
                                   label="结束时间">
                        <template slot-scope="scope">
                            {{scope.row.changdiJieshuTime}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="changdiYuyueYesnoTypes"
                                      header-align="center"
                                      label="预约状态">
                        <template slot-scope="scope">
                            {{scope.row.changdiYuyueYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="changdiYuyueYesnoText"
                                      header-align="center"
                                      label="审核回复">
                        <template slot-scope="scope">
                            <span v-if="scope.row.changdiYuyueYesnoText != null &&scope.row.changdiYuyueYesnoText.length>10">
                                {{scope.row.changdiYuyueYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.changdiYuyueYesnoText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('changdiYuyue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('changdiYuyue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('changdiYuyue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('changdiYuyue','审核') && scope.row.changdiYuyueYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              changdiYuyueYesnoTypesSelectSearch : [],
            form:{
                id : null,
                changdiYuyueUuidNumber : null,
                yonghuId : null,
                changdiId : null,
                sheshiyuyueText : null,
                changdiYuyueTime : null,
                changdiJieshuTime : null,
                changdiYuyueYesnoTypes : null,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '场地照片': 'changdiPhoto',
                     '场地类型': 'changdiTypes',
                     '场地容量': 'changdiKucunNumber',
                     '价格/分钟': 'changdiNewMoney',
                     '场地热度': 'changdiClicknum',
                     '是否上架': 'shangxiaTypes',
                     '用户名称': 'yonghuName',
                     '用户手机号': 'yonghuPhone',
                     ...
                     '现有余额': 'newMoney',
                //本表字段
                     '预约编号': "changdiYuyueUuidNumber",
                     '备注': "sheshiyuyueText",
                     '开始时间': "changdiYuyueTime",
                     '结束时间': "changdiJieshuTime",
                     '预约状态': "changdiYuyueYesnoValue",
                     '审核回复': "changdiYuyueYesnoText",
                     '申请时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'changdi_yuyue',//当前表表名,
                        // sumColum : 'changdi_yuyue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'changdi_yuyue_name',//分组字符串字段
                        types : 'changdi_yuyue_yesno_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "changdi_yuyue_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.changdiYuyueUuidNumber!= '' && this.searchForm.changdiYuyueUuidNumber!= undefined) {
                    params['changdiYuyueUuidNumber'] = '%' + this.searchForm.changdiYuyueUuidNumber + '%'
                }
                                                 
                if (this.searchForm.changdiYuyueYesnoTypes!= '' && this.searchForm.changdiYuyueYesnoTypes!= undefined) {
                    params['changdiYuyueYesnoTypes'] = this.searchForm.changdiYuyueYesnoTypes
                }
                        
                params['changdiYuyueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=changdi_yuyue_yesno_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.changdiYuyueYesnoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            changdiYuyueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "changdiYuyue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入场地预约申请数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        ...
                        data: {
                            id:id,
//                            changdiYuyueTypes:changdiYuyueTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
