        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/jianshenfangguanli/upload/changdiMuBan.xls"
                        >批量导入场地信息数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('changdi','导入导出')"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="changdiKucunNumber"
                                      header-align="center"
                                      label="场地容量">
                        <template slot-scope="scope">
                            {{scope.row.changdiKucunNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <!--<el-button v-if="isAuth('changdi','修改')" type="primary" icon="el-icon-add-location" size="mini" @click="plusChangdiKucunNumber(scope.row.id,scope.row.changdiKucunNumber)">增加场地容量</el-button>-->
                            <!--<el-button v-if="isAuth('changdi','修改')" type="primary" icon="el-icon-delete-location" size="mini" @click="reduceChangdiKucunNumber(scope.row.id,scope.row.changdiKucunNumber)">减少场地容量</el-button>-->
...
<!-- 库存 -->
		<el-dialog
                :title="changdiKucunNumberTitle"
                :visible.sync="changdiKucunNumberVisible"
                width="30%">
            <span>操作数量</span><el-input-number v-model="kucunNumber" :min="1" :max="100" label="描述文字"></el-input-number>
            <span slot="footer" class="dialog-footer">
			<el-button @click="changdiKucunNumberVisible = false">取 消</el-button>
			<el-button type="primary" @click="addChangdiKucunNumber()">确 定</el-button>
		  </span>
        </el-dialog>
...
<script>
    ...
    //当前表下拉框搜索条件
        ...
        //库存
            ...
            //原有数量
            changdiKucunNumber: 0,
            //标题
            changdiKucunNumberTitle: null,
            //模态框状态
            changdiKucunNumberVisible: false,
            };
        },
        created() {
        ...
        methods: {
            ...
            //增加场地容量
			plusChangdiKucunNumber(thisId,changdiKucunNumber){
			//清空数据
				//场地容量数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				//将模态框打开
				this.changdiKucunNumberVisible = true;
				//标题
				this.changdiKucunNumberTitle = '增加场地容量'
			},
			//减少场地容量
			reduceChangdiKucunNumber(thisId,changdiKucunNumber){
			//清空数据
				//场地容量数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				this.changdiKucunNumberVisible = true;
				//标题
				this.changdiKucunNumberTitle = '减少场地容量'
			},
			//提交用户操作场地容量数量
			addChangdiKucunNumber(){
				var type = null
				var maxNumber = 0
				if(this.kucunNumberTypes == "+"){
					type = '增加'
					maxNumber = this.changdiKucunNumber + this.kucunNumber
				}else{
					type = '减少'
					if(this.changdiKucunNumber < this.kucunNumber || this.kucunNumber <= 0){
						this.$message.error('场地容量不足不能减少');
						return false;
					}else{
						maxNumber = this.changdiKucunNumber - this.kucunNumber
					}
				...
				this.changdiKucunNumberVisible = false;
			},	
			//上下架
