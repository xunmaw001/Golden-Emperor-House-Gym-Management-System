...
<script>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                   ...
                   zhuchangId: [
                              { required: true, message: '驻场人员不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
        ...
        created() {
         ...
         this.$http({
             url: `dianzhang/page?page=1&limit=100`,
             method: "get"
         }).then(({ data }) => {
             if (data && data.code === 0) {
                this.dianzhangOptions = data.data.list;
            }
         });
         this.$http({
         ...
         this.$http({
             url: `zhuchang/page?page=1&limit=100`,
             method: "get"
         }).then(({ data }) => {
             if (data && data.code === 0) {
                this.zhuchangOptions = data.data.list;
            }
         });
