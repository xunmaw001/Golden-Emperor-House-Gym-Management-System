    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(forum !=null){
            ...
            //级联表 店长
            //级联表
            DianzhangEntity dianzhang = dianzhangService.selectById(forum.getDianzhangId());
            if(dianzhang != null){
            BeanUtils.copyProperties( dianzhang , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"
, "dianzhangId"
, "jiaolianId"
, "qiantaiId"
, "zhuchangId"
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setDianzhangId(dianzhang.getId());
            }
            //级联表 前台
            //级联表
            QiantaiEntity qiantai = qiantaiService.selectById(forum.getQiantaiId());
            if(qiantai != null){
            BeanUtils.copyProperties( qiantai , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"
, "dianzhangId"
, "jiaolianId"
, "qiantaiId"
, "zhuchangId"
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 驻场人员
            //级联表
            ZhuchangEntity zhuchang = zhuchangService.selectById(forum.getZhuchangId());
            if(zhuchang != null){
            BeanUtils.copyProperties( zhuchang , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"
, "dianzhangId"
, "jiaolianId"
, "qiantaiId"
, "zhuchangId"
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setZhuchangId(zhuchang.getId());
            }
            //级联表 用户
            //级联表
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setDianzhangId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        else if("教练".equals(role))
            forum.setJiaolianId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        else if("前台".equals(role))
            forum.setQiantaiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        else if("驻场人员".equals(role))
            forum.setZhuchangId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            ...
            .eq("dianzhang_id", forum.getDianzhangId())
            .eq("qiantai_id", forum.getQiantaiId())
            .eq("zhuchang_id", forum.getZhuchangId())
...
//                            forumEntity.setDianzhangId(Integer.valueOf(data.get(0)));   //店长 要改的
//                            forumEntity.setQiantaiId(Integer.valueOf(data.get(0)));   //前台 要改的
//                            forumEntity.setZhuchangId(Integer.valueOf(data.get(0)));   //驻场人员 要改的
